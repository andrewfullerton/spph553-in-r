...
This is because SQL is not integrated into R via `sqldf` the same way that `PROC SQL` integrates SQL with SAS. We cannot read and write data into/from our R environment using `sqldf`. 
...
Try to find a function that will this shapefile into R. Don't load any packages! Hint: most base R read functions begin with `read`.
